...
function setCookie ( obj,jsonUrl ){
  ...
  Cookies.set( 'parent_page_type', '0601', {
    path:'/'
  });
  ...
  getsearchRecommend(function( res ){
    ...
    if( rec ){
      ...
      if( history_Content.history_list.length ){
        ...
        if( history_Content.vProBlock ){
          history_Content.domTransition( 'vProBlock','top', ( ( history_Content.pos + 1 ) / ( history_Content.history_list.length + 1 )) * 100 + '%' );
        }
      }
      ...
      if( history_Content.history_list.length ){
        ...
        if( history_Content.vProBlock ){
          history_Content.domTransition( 'vProBlock','top', ( history_Content.pos / ( history_Content.history_list.length + 1 )) * 100 + '%' );
        }
      } else {
        addClass( getElement('.yh_list_item-' + history_recommend.itemNo),'hover' );
        areaObj = history_recommend;
...
userTrackList( userObj,function(res){
  ...
  if( window.location.href.indexOf('?') != -1 ){
    ...
    if( tagArr ){
      var exist = getElement('.yh_list_item-' + history_Content.pos +'-'+ history_Content.curPageIndex +'-' + history_Content.itemNo);
...
history_Content.init = function(){
  ...
  if( this.vProBlock ){
    this.domTransition( 'vProBlock','top', ( this.pos / ( this.history_list.length + 1 )) * 100 + '%' );
  }
  if( this.curProBlock ){
...
history_Content.template = function(){
  if ( this.history_list.length ){
    ...
    var _this = this;
    if( this.history_list.length ){
      vertical_scrollBar.init( 1 ,this.history_list.length + 1 );
    }
...
history_Content.down = function(){
  ...
  if( this.pos < this.history_list.length - 1 ){
    ...
    areaObj = history_recommend;
    if( this.vProBlock ){
      this.domTransition( 'vProBlock','top', ( this.pos + 1  ) / ( this.history_list.length + 1 ) * 100 + '%' );
    }
    removeClass( getElement('.yh_list_item-' + this.pos +'-'+ this.curPageIndex +'-' + this.itemNo),'hover' );
    addClass( getElement('.yh_list_item-' + history_recommend.itemNo),'hover' );
  }
...
vertical_scrollBar.init = function( pIndex , pSize ){
  this.parent = getElement('#history');
