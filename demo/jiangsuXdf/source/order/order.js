...
bi.pageJump( Cookies.get('parent_page_id'), '101' );
...
order_menu_list.init = function(){
  ...
  addClass( getElement('.order_pos'), 'hover' );
}
...
order_menu_list.right = function(){
  ...
  removeClass( getElement('.order_pos'), 'hover' );
  removeClass( getElement('.order_menu_item-'+ this.itemNo),'hover' );
  removeClass( getElement('.order_list_item-'+ order_type.itemNo),'tag' )
  addClass( getElement('.order_menu_item-'+ this.itemNo),'tag_hover' );
  addClass(getElement('.order_list_item-'+ order_type.itemNo),'hover');
  addClass( getElement('.order_type'),'hover' )
};
...
order_type.template = function(){
  ...
  order_typeInfo && order_typeInfo.forEach(function( item,index ){
    var dom = '<div class="order_list_item order_list_item-'+ item.id +'">'+
                    item.price +
                  '<div class ="count_price">'+item.count+'<div class ="line"></div>'+ '</div>'
                  + 
                  '<i class="order_item_sign"></i>'+
                  '<div class ="order_type_bg"></div>' +
              '</div>';
    list += dom;
  })
...
order_type.up = function(){
  if( this.pos == 1 ){
    ...
    removeClass( getElement('.order_list_item-'+ this.itemNo),'tag' )
  } else {
...
order_type.down = function(){
  if( this.pos == 0 ){
    ...
    removeClass( getElement('.order_type'),'hover' );
    addClass( getElement('.order_list_item-'+ this.itemNo),'tag' )
  } else {
  ...
  if( flag ){
    ...
    if( this.pos == 0 ){
      if( this.itemNo > 0 ){
        ...
        removeClass(getElement('.order_list_item-'+ this.itemNo),'hover');
        addClass( getElement('.order_pos'), 'hover' );
        removeClass( getElement('.order_menu_item-'+ order_menu_list.itemNo),'tag_hover' );
        addClass( getElement('.order_menu_item-'+ order_menu_list.itemNo),'hover' );
        removeClass( getElement('.order_type'),'hover' );
        addClass( getElement('.order_list_item-'+ this.itemNo),'tag' )
        return;
      }
...
onKeyPress = function (keyCode) {
  switch (keyCode) {
    case "back":
      var orderbackUrl = Cookies.get('orderbackUrl') || '../../index.html'
      window.location.href = orderbackUrl;
      break;
    case "up": //上边
    areaObj && areaObj.up();
      break;
