...
var value = {
    ...
    getValue: function() {
        if (window.location.search.length > 1) {
            this.currClass = Number(unity.getVars("itemNo")); console.log(this.currClass)
...
var getData = function(pindex, psize) {
    ...
    if (value.currClass == 1) {
        ...
        unity.qeryAllHistory(pindex * 8,psize,function(res){
            console.log(JSON.stringify(res))
            console.log("历史res"+JSON.stringify(res))
            payList = res.data.resultList
            console.log(payList);
        if (payList.length > 0) {
            value.totalpage = (res.data.resultNum && (Math.ceil(res.data.resultNum / 8)));
            value.totalNum = res.data.resultNum;
            value.detailData = res.data.resultList;
            $(".his-content ul").empty();
            console.log(JSON.stringify(value.detailData));
            for (var j = 0; j < value.detailData.length; j++) {
                ...
                $(".his-content ul").append('<li><div class="imgbox"><div class="score"><span>' + value.detailData[j].relateScore + '</span></div><img class="lazyload" src="../public/images/img_loading_160x230.png"  data-img="' + value.detailData[j].relateImg + '"/></div><div class="word">' + value.detailData[j].relateTitle + '</div></div></li>')
            }
            $.imgLazyLoad();
            if (value.totalpage >= 1) {
                $(".total-page").html(value.pindex + 1 + '/' + value.totalpage);
    ...
    enter: function() {
        if (value.itemNo == -1) {
            if (value.currClass == 1) {
                ...
                console.log('清空历史');
                unity.delAllHistory(function(res){
                    $('.his-clear').removeClass('writediv');
                $(".total-page").html('');
                $(".search-scroll").css({
                    "visibility": "hidden"
            ...
            try {
				var jsonOb = {}  //jsonOb = {page_type: "0501", cid: 202082, click_type: "1"}
				if (value.currClass == 2) {//历史
                    jsonOb.page_type = '0601'  //jsonOb = {page_type: "0501", cid: 202082, click_type: "1"}
					jsonOb.cid = value.detailData[value.itemNo].relateId
					jsonOb.click_type = '1'
				} else {
					jsonOb.page_type = '0501'//收藏
					jsonOb.cid = value.detailData[value.itemNo].relateId
					jsonOb.click_type = '1'
				}
                bi.historical(jsonOb)
            ...
            $(cookieHistory());
            var url = "historyHome.html?itemNo=" + value.currClass + "&pindex=" + value.pindex + "&contentItemNo=" + value.itemNo;  //url = "historyHome.html?itemNo=1&pindex=0&contentItemNo=0"
            ...
            if (value.currClass == 1) {
                $.cookie('detailUrl', value.detailData[value.itemNo].relateUrl, { path: '/' });  //preURL = "../historyHome/historyHome.html?itemNo=1&pindex=0&contentItemNo=0"
            } else {
                var relateUrl = 'http://116.62.93.251/?s=42|17&p=yhAssetDetail&k=1&v=1&c=2&assetId=' + value.detailData[value.itemNo].relateId; //relateUrl = "http://116.62.93.251/?s=42|17&p=yhAssetDetail&k=1&v=1&c=2&assetId=960556"
                $.cookie('detailUrl', relateUrl, { path: '/' });
              //  paydetail = unity.getPayDetail(value.detailData[value.itemNo].relateId);
                 unity.qeryAllHistory(0,1,function(res){
                    paydetail = res
                })
            }
