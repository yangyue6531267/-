...
var _SVPInfo = {
  getSTBID: function (){
    return starVGf.getCookie('STARV_UserID') +
					starVGl.sign + starVGf.getCookie('STARV_Mac');
  },
  getHBInfo: function (){
    return starVGl.sign + 'H' + starVGl.sign + '0' + starVGl.sign + starVGf.getCookie('STARV_OperatorID') +
				     starVGl.sign + starVGl.lastServiceFlag + starVGl.sign + starVGf.getCookie('STARV_ApkVersion') + starVGl.sign + starVGf.getCookie('STARV_ReserveGroup') +
					 starVGl.sign + starVGf.getCookie('STARV_CollectType') + starVGl.sign + starVGf.getCookie('STARV_TerraceID') +
					  starVGl.sign + starVGf.getCookie('STARV_Model');
  },
...
var starvAjax2 = function (url, cfg) {
  ...
  if (xhr != null) {
    ...
    xhr.open(type.toUpperCase(), uri.indexOf('?') == -1 ? uri + '?ver=' + Math.random() : uri, true);
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4) {
        ...
        if (dataType != undefined && dataType.toLowerCase() == 'json') xhrRespText = eval('(' + xhrRespText.replace(/(^\s*)|(\s*$)/g, '').replace(/^\s*|\s*$/g, '') + ')');
        if (dataType != undefined && dataType.toLowerCase() == 'jscript') xhrRespText = eval(xhrRespText.replace(/(^\s*)|(\s*$)/g, '').replace(/^\s*|\s*$/g, ''));
        if (xhr.status == 200) {
...
function doSendPlayData1 (){
  ...
  try {
    if (type == '200') { // 心跳接口
      if (starVGl.currP.info.Switch == 0){ // 关闭心跳接口
        starvp('starvCK if switch 00 starVGf.getCookie("STARV_openHeart") = ' + starVGf.getCookie('STARV_openHeart'));
        ...
        starvp('starvCK else switch 11 starVGf.getCookie("STARV_openHeart") = ' + starVGf.getCookie('STARV_openHeart'));
...
var starvData = { // 组装数据
  getData: function (t, info){
    ...
    var str = '';
    if (t == '255'){
      str = t + starVGl.sign + _SVPInfo.getSTBID() + starVGl.sign + __SUtil.getDateTime() + starVGl.sign + '';
    } else {
      ...
      switch (t) {
        case '255':// Heartbeat
          /*
					 ...
					 */
          str += _SVPInfo.getHBInfo() + starVGl.sign + starVGl.currName;
          break;
        case '01':/* boot */
          str = t + starVGl.sign + info.UserID + starVGl.sign + info.Mac + starVGl.sign + __SUtil.getDateTime() +
								 starVGl.sign + info.UserID + starVGl.sign + info.OperatorID +
								 starVGl.sign + info.TerraceID + starVGl.sign + info.ContentID +
								 starVGl.sign + info.Brand + starVGl.sign + info.Mode + starVGl.sign + info.ApkVersion +
								 starVGl.sign + info.ReserveGroup + starVGl.sign + info.CollectType + starVGl.sign + info.AppKey +
								 starVGl.sign + info.Reserve1 + starVGl.sign + info.Reserve2;
          break;
        case '06':/* page view or page click */
          starVGl.lastServiceFlag = '06';
          starVGl.currName = decodeURIComponent(info.PageName);
          str += starVGl.sign + info.PagePath +
							 starVGl.sign + info.SrcPagePath +
							 starVGl.sign + decodeURIComponent(info.PageName) +
							 starVGl.sign + info.AssetId +
							 starVGl.sign + info.Id +
							 starVGl.sign + info.TypeId +
							 starVGl.sign + info.OperatorID;
          break;
        case '07':/* order */
          starVGl.lastServiceFlag = '07';
          starVGl.currName = '';
          str += starVGl.sign + info.ProductType +
							starVGl.sign + info.ProductID +
							starVGl.sign + decodeURIComponent(info.ProductName) +
							starVGl.sign + info.ProviderID +
							starVGl.sign + info.EndTime +
							starVGl.sign + info.Action +
							starVGl.sign + info.OrderStatus +
							starVGl.sign + info.Fail +
							starVGl.sign + info.OrderUrl +
							starVGl.sign + info.Confirmation +
							starVGl.sign + info.UrlName +
							starVGl.sign + info.Reserve1 +
						 	starVGl.sign + info.Reserve2 +
							 starVGl.sign + info.OperatorID;
          break;
        case '03':/* vod */
          starVGl.currName = decodeURIComponent(info.FilmName);
          starVGl.lastServiceFlag = '109';
          str += starVGl.sign + info.ColumnID +
							starVGl.sign + decodeURIComponent(info.ColumnName) +
							starVGl.sign + info.AssetID +
							starVGl.sign + decodeURIComponent(info.FilmName) +
							starVGl.sign + info.Episodes +
							starVGl.sign + info.VodEpisodes +
							starVGl.sign + info.OrderStatus +
							starVGl.sign + info.VodTime +
							starVGl.sign + info.VodOperateType +
							starVGl.sign + info.Pace +
							starVGl.sign + info.EnterType +
							starVGl.sign + info.FilmType +
							starVGl.sign + info.StartTime +
							starVGl.sign + info.State +
							starVGl.sign + info.ProductID +
							starVGl.sign + info.SPID +
							starVGl.sign + info.Reserve1 +
						    starVGl.sign + info.Reserve2 +
							 starVGl.sign + info.OperatorID;
          break;
        case '08':/* fav */
          ...
          str += starVGl.sign + info.ChannelID +
							 starVGl.sign + decodeURIComponent(info.ChannelName) +
							 starVGl.sign + info.ChannelStatus +
							 starVGl.sign + info.Reserve1 +
							 starVGl.sign + info.Reserve2 +
							 starVGl.sign + info.OperatorID;
        ...
        case '09':/* yuding */
          ...
          str += starVGl.sign + info.ChannelID +
							 starVGl.sign + decodeURIComponent(info.ChannelName) +
							 starVGl.sign + info.AD_ColumnID +
							 starVGl.sign + info.AsssetID +
							 starVGl.sign + decodeURIComponent(info.AsssetName) +
							 starVGl.sign + info.ChannelStatus +
							 starVGl.sign + info.ReserveType +
							 starVGl.sign + info.Reserve1 +
							 starVGl.sign + info.Reserve2 +
							 starVGl.sign + info.OperatorID;
          break;
        case '10':/* app */
          ...
          str += starVGl.sign + info.ApplylID +
							 starVGl.sign + info.ApplylName +
							 starVGl.sign + info.ApplyColumnID +
							 starVGl.sign + decodeURIComponent(info.ApplyColumnName) +
							 starVGl.sign + info.ApplylStatus +
							 starVGl.sign + info.EnterType +
							 starVGl.sign + info.ApplyType +
							 starVGl.sign + info.Reserve1 +
							 starVGl.sign + info.Reserve2 +
							 starVGl.sign + info.OperatorID;
        ...
        case '11':/* ad */
          ...
          str += starVGl.sign + info.AdID +
							 starVGl.sign + info.AdName +
							 starVGl.sign + info.AdColumnID +
							 starVGl.sign + decodeURIComponent(info.AdColumnName) +
							 starVGl.sign + info.AdlStatus +
							 starVGl.sign + info.EnterType +
							 starVGl.sign + info.ApplyType +
							 starVGl.sign + info.State +
							 starVGl.sign + info.Reserve1 +
        ...
        case '20':/** content_info */
          str += info.Program_name +
							 starVGl.sign + info.Product_code +
							 starVGl.sign + info.Product_name +
							 starVGl.sign + info.director +
							 starVGl.sign + info.actor +
							 starVGl.sign + info.year +
							 starVGl.sign + info.region +
							 starVGl.sign + info.label +
							 starVGl.sign + info.program_type +
							 starVGl.sign + info.program_status +
							 starVGl.sign + info.program_online_time +
							 starVGl.sign + info.program_offline_time;
          break;
        case '21':/** user_vod */
          str += info.date +
							 starVGl.sign + info.tv_no +
							 starVGl.sign + info.zone_no +
							 starVGl.sign + info.Program_code +
							 starVGl.sign + info.Program_type +
							 starVGl.sign + info.Play_id +
							 starVGl.sign + info.Event_type +
							 starVGl.sign + info.time +
							 starVGl.sign + info.ip;
        ...
        case '22':/** panel_click */
          str += info.date +
							 ...
							 starVGl.sign + info.zone_no +
							 starVGl.sign + info.ip +
							 starVGl.sign + info.page_name +
							 starVGl.sign + info.page_level +
							 starVGl.sign + info.Panel_id +
							 starVGl.sign + info.Recommend_id +
							 starVGl.sign + info.Page_url +
							 starVGl.sign + info.Load_time +
							 starVGl.sign + info.Load_status;
        ...
        case '23':/** exposure */
          str += info.date +
							 starVGl.sign + info.tv_no +
							 starVGl.sign + info.zone_no +
							 starVGl.sign + info.Exposure_id +
							 starVGl.sign + info.Exposure_type +
							 starVGl.sign + info.Exposure_content_id +
							 starVGl.sign + info.Exposure_content_type +
							 starVGl.sign + info.Exposure_start_time +
							 starVGl.sign + info.Exposure_end_time;
          break;
        case '100':/* group */
  ...
  sendData: function (url, d, t, cb){ // 发送数据
    starvp('starvCK send data = ' + d);
    if (!d || (d.indexOf('|') != -1 && d.split('|')[2] == 'NO')){ return 0; }
    if (!starVGf.getNeedSend(d.split('|')[2])){ return 0; }
    if (starVGf.getCookie('STARV_openHeart') == 0 && d.split('|')[0] == 255){ return 0 };
    // var _url = url+"?" + d;
    var _url = '';
    if (t === '20'){
...
function __starVinit (){ // 启动心跳定时任务
  var murl = window.location.href.indexOf('?') != -1 ? window.location.href.split('?')[0] : window.location.href;
  ...
  starvIntervalTimeout = setInterval(function (){
    starvData.sendData(starvServerURL, starvData.getData('255'), '255');
  }, 5 * 60 * 1000);
}

var starVready = function (fn){ // 页面初始化
