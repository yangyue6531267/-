...
if (getQueryString('topicId')) {
  //外链跳转专题页面
  special.url = baseUrl + 'p=yhSpecialDetail&k=1&v=1&topicId=' + getQueryString("topicId") + '&c=13'
  init();
  try {
    bi.start('0201');
  } catch (error) {
    ...
    special.url, function (response) {
      ...
      if(jump) {
        ...
        try {
          ...
          jsonOb.page_id = eval('(' + response + ')').data.specialId
