...
// var poweruUrl = "http://117.131.17.59:8095/scspProxy";//订购鉴权正式(需要开通白名单)
// var poweruUrl = "http://112.17.251.186:10089/hainanorder";//正式代理
var userData = {
  payType: "16",//1.支付宝 2:微信 4. 和包 8.手机话费老五套支付 16.阳光计划新六套支付
  subType: "03",
}
// 判断类型的方法
  ...
  ajax({
    ...
    success: function (res) {
      ...
      for (var i = 0; i < productDom.length; i++) {
        var obj = {};
        obj.originalPrice = '10'

        obj.id = i;
        obj.productName = productDom[i].getAttribute('productInfo');
        obj.productCode = productDom[i].getAttribute('productCode');
        obj.orderContentId = productDom[i].getAttribute('orderContentId');
        obj.pic = productDom[i].getAttribute('price');
        ...
        obj.type = 1;//1实际绑定的包，0虚拟出来的扫码包月 2话费包月
        if (productDom[i].getAttribute('unit') == 2 || productDom[i].getAttribute('unit') == 3) {
          obj.type = 2;//1实际绑定的包，0虚拟出来的扫码包月
          obj.title = unit;
        } else {
          obj.title = productDom[i].getAttribute('cycle') + unit;
        }
        value.productList.push(obj);
        //////////////话费包月和连续包月分开处理，所以多一个单月包(产品经理需求)
        if (productDom[i].getAttribute('unit') == 2) {
          var data = JSON.parse(JSON.stringify(obj))
          data.type = 0;
          data.title = "包月";
          value.productList.push(data);
        }
      }
      callBack && callBack();
    },
