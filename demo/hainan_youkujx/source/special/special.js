...
var getData = function (num) {
  unity.jsonp(value.specialUrl, function (res) {
    ...
    for (i in value.detailData.elementList) {
      // $(".scroll-box").append('<li><div class="imgbox"><div class="score"><span>' + value.detailData.elementList[i].score + '</span></div><img class="lazyload" src="../public/images/img_loading_160x230.png"  data-img="' + value.detailData.elementList[i].elementImg + '"/></div><div class="word">' + value.detailData.elementList[i].elementName + '</div></div></li>')
      $(".scroll-box").append('<li><div class="imgbox"><div class="score"><span>' + value.detailData.elementList[i].score + '</span></div><img class="lazyload" src="' + value.detailData.elementList[i].elementImg + '"/></div><div class="word">' + value.detailData.elementList[i].elementName + '</div></div></li>')
    }
    //$.imgLazyLoad();
    special.addCss();
...
var special = {
  ...
  down: function () {
    if (value.detailData.elementList.length > 6 && value.detailData.elementList.length < 13) { //两
      ...
      if (this.itemNo > 11) return; // parseInt(value.detailData.elementList.length/6)*6
      this.removeCss();
      $('.middle-box').css("top", "-170px")
