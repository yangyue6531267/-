...
var totalCard = orderList.length;
var orderNumber;
var orderTime;
var validFrom;
var validTo;
...
var baseUrl2 = 'http://gsyd-ds.yanhuamedia.tv/'; // 正式代理
...
function getToken() {
  $.ajax({
    type: 'POST',
    url: baseUrl2+"boms/v1/access_token",
    dataType: 'json',
    headers: {
      powerkey: powerKey
    },
...
function orderCode() {
  ...
  qrOrderId = 'T' + deviceObj.stbId.substring(deviceObj.stbId.length - 2) + Math.round(new Date().getTime() / 1000).toString()+Math.ceil(Math.random()*99);
  // var qrOrderId = 'T1234567893'
  var url = baseUrl2 + 'v3/gs/ccb/qr_code?qrOrderId=' + qrOrderId;
  // var url =  "http://117.156.24.246:8990/v2/gs/ccb/qr_code?qrOrderId="+ qrOrderId
  var params = {
    "body": {
      "idType": 0, //用户标识类型，0:手机号,1:宽带账号
      "idValue": mobileNos, //手机号
      "deviceType": 1, //设备类型，0：智能网关,1：机顶盒,2：摄像头,3：其它。如果无设备，则选3.[1 机顶盒]
      "deviceId": deviceObj.stbId, //设备MAC，订购需要设备信息时，必选
      // productId: orderList[hoverIndex].productId, //产品标识，家开平台定义的产品或套餐编号
      "productId": productId, //产品标识，家开平台定义的产品或套餐编号
      "orderChannel": 4, //01	手机客户端	业务平台 02 WAP 业务平台 03 SMS 业务平台 04 机顶盒 业务平台5 智能网关 业务平台06 网上营业厅 BOSS 08 短信营业厅 BOSS 11 营业前台 BOSS 12 WWW网站 业务平台 13 PC客户端 业务平台
  ...
  //     },
  //        "header":{
  //         "access_token":"6adbe8d666f547ad9810ec83c8af99a5",
  //            "msgid":"61237890345",
...
//正式代理环境
  var params1 = {action:"1",platformCode:"p0008",version:"1",accountId:mobileNos}
  var userTokens = '';
  var userId = '';
  var usertoken = $.ajax({
    type: 'POST',
...
function order() {
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      var url = baseUrl2 + 'v2/gs/tel/order';  //新版本接口未上线，上线时间待定
      ...
      var params = {
        "body": {
          ...
          "idValue": mobileNos, //手机号
          "deviceType": 1, //设备类型，0：智能网关,1：机顶盒,2：摄像头,3：其它。如果无设备，则选3.[1 机顶盒] 
          // "deviceId": '005803FF001827800005748F1B4F7B46', //设备MAC，订购需要设备信息时，必选
          "mac":deviceObj.stbId,//mac
          "productId": productId, //产品标识，家开平台定义的产品或套餐编号
          // "chargeType": orderList[hoverIndex].packageType, //计费类型，1:免费；2：按次；3：包月；4：包年计费；5：按时长计费；6：按周期计费。参考附录C 计费类型编码。
          "chargeType": chargeType, //计费类型，1:免费；2：按次；3：包月；4：包年计费；5：按时长计费；6：按周期计费。参考附录C 计费类型编码。
          "orderChannel": 4, //01	手机客户端	业务平台 02 WAP 业务平台 03 SMS 业务平台 04 机顶盒 业务平台5 智能网关 业务平台06 网上营业厅 BOSS 08 短信营业厅 BOSS 11 营业前台 BOSS 12 WWW网站 业务平台 13 PC客户端 业务平台
...
function orderwork() {
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      console.log(userTokens);
      var params ={
        platformCode:"33",//业务平台
        transactionId: orderNumber,//订单号
        accountId:mobileNos,//y用户账户
        userId:userId,//用户id
        userToken:userTokens,//用户令牌
        mac:deviceObj.stbId,//mac
        // mac:"005803FF001827800005748F1B4F7B46",//mac
        productCode:productId,//产品编码
      ...
      console.log(params);
          var orderfor = $.ajax({
            type: 'POST',
            url: baseUrl1+'/bms/p/order/gsxdfMPay',
            imeout: 5000,
            data:JSON.stringify(params),
            success: function (data) {
              console.log(data);
