...
var getData = function (url) {
  ...
  unity.jsonp(url, function (res) {
    ...
    var contentId = value.detailData.itemList[0].vodList[0].playUrl;
    unity.userPower(upluadDom, contentId, value.detailData.assetName);
  }, function (err) {
    unity.alertMsg("资产已下线，即将返回上一页。")
    setTimeout(function () { //三秒返回上一个
      var url = $.cookie("preURL")
...
var topContent = {
  ...
  enter: function () {
    if (this.btnNum === 0) {
      ...
      var url = "../player/player.html?fromPage=movieDetail.html&contentNum=0&g_playlong=0"
      window.location = url
