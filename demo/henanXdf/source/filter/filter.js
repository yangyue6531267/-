...
function init() {
  if (window.location.search.length > 1) {
    ...
    getConditionFilter(getQueryString("catCode"), function (res) {
      if (res.code == 200) {
        filter.fliterList = res.data.categoryList[0].children;
        filter.createFliter(filter.fliterList, res.data.level);
      }
    })
...
filter.filterData = function (pindex, categoryId, state) {
  ...
  getFilterResult(url, function (response) {
    ...
    if (response.code !== 200) {
      ...
      filter.template(response.data);
      connentBox.props(response.data)
    }
  }, function (error) {
  })
}
