...
connentBox.down = function () {
  if (this.pindex >= filter.totalPage && (this.pindex - 1) * 8 + this.itemNo + 4 >= filter.totalNum) {
    ...
    this.pindex++;
    this.movebar(this.pindex);
    filter.filterData(this.pindex, filter.categoryId);
  } else {
    removeClass(this.getCurr(), 'hover');
    connentBox.marquee();
    if (this.itemNo < 4) {
...
connentBox.enter = function () {
  ...
  var url = "../filter/filter.html?catCode=" + filter.itemNo + "&itemNo0=" + filter.itemNo0 + "&itemNo1=" + filter.itemNo1 + "&itemNo2=" + filter.itemNo2 + "&pindex=" + this.pindex + "&contentItemNo=" + this.itemNo;
