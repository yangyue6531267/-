 ...
 */
	...
	var _SVPInfo = {
		...
		setSTBInfo : function(o){
			starVGf.setCookie("STARV_Mac",o.Mac);
			starVGf.setCookie("STARV_UserID",o.UserID);
			starVGf.setCookie("STARV_OperatorID",o.OperatorID);
			starVGf.setCookie("STARV_ReserveGroup",o.ReserveGroup);
			starVGf.setCookie("STARV_CollectType",o.CollectType);
			starVGf.setCookie("STARV_ApkVersion",o.ApkVersion);
			starVGf.setCookie("STARV_TerraceID",o.TerraceID);
			starVGf.setCookie("STARV_Model",o.Mode);
	...
	var starvData = {
		getData:function(t,info){
			...
			if(starVGl.debug)starvData.sendData(starvServerURL,"getData t= " +  t)
			try {
				switch (t) {
					case "255"://Heartbeat
					/*
		...
		sendData:function(url,d,cb){
			...
			try {
				XHA_STATISTICS.setXha_statistics(_url, "xhaLogs");
			} catch (error) {
				console.log("星红桉采集数据发送调用失败:HTTP_BRIDGE.get");
			}
	...
	function starvdoSend(url,t,i){
		starvData.sendData(url,starvData.getData(t,i));
	}
	...
	function __starVinit(){
		...
		starVGl.isPortal = (murl.substr(murl.length-23)== starVGl.portalURL?true:false);
		...
		starvIntervalTimeout = setInterval(function(){
				starvData.sendData(starvServerURL, starvData.getData("255"));	
		}, 5 * 60 * 1000);
