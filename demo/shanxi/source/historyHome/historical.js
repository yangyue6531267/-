    ...
    if (value.currClass == 1) {
        unity.collectData(pindex * 8, psize, function(res) {
            ...
            for (var i = 0; i < value.detailData.length; i++) {
                $(".his-content ul").append('<li><div class="imgbox"><div class="score"><span>' + value.detailData[i].relateScore + '</span></div><img class="lazyload" src="../public/images/1img_loading_160x230.png"  data-img="' + value.detailData[i].relateImg + '"/></div><div class="word">' + value.detailData[i].relateTitle + '</div></div></li>')
            }
            $.imgLazyLoad();
            //页数
        ...
        if (payList.length > 0) {
            ...
            for (var j = 0; j < value.detailData.length; j++) {
                $(".his-content ul").append('<li><div class="imgbox"><div class="score"><span>' + value.detailData[j].score + '</span></div><img class="lazyload" src="../public/images/1img_loading_160x230.png"  data-img="' + value.detailData[j].assetImg + '"/></div><div class="word">' + value.detailData[j].assetName + '</div></div></li>')
            }
            $.imgLazyLoad();
            if (value.totalpage >= 1) {
                $(".total-page").html(value.pindex + 1 + '/' + value.totalpage);
    ...
    enter: function() {
        if (value.itemNo == -1) {
            ...
            if (value.currClass == 1) {
                ...
                var relateUrl = 'http://112.17.251.186:10088/?s=43|27&p=yhAssetDetail&k=1&v=1&c=2&assetId=' + value.detailData[value.itemNo].assetId;
