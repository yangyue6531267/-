...
function init() {
  ...
  if (window.location.search.length > 1) {
    filter.itemNo = getParam("catCode");
    getConditionFilter(getParam("catCode"), function (res) {
    ...
    if (getParam("pindex")) {
      filter.isBack = true;
      filter.itemNo0 = Number(getParam("itemNo0"));
      filter.itemNo1 = Number(getParam("itemNo1"));
      filter.itemNo2 = Number(getParam("itemNo2"));
...
filter.createFliter = function (obj, row) {
  if (row == 4) {
    ...
    if (filter.fliterList[filter.itemNo0].children == undefined) {
      ...
      if (filter.fliterList[filter.itemNo0].children[filter.itemNo1].children == undefined) {
        categoryId = filter.fliterList[filter.itemNo0].children[filter.itemNo1].categoryId
        filter.filterData(filter.pindex, categoryId);
        filter1.innerHTML = '';
...
filter.gather = function (obj, item, row) {
  ...
  filter.filterData(connentBox.pindex, condition);
}
filter.template = function (obj) {
  ...
  lazyLoadImage();
  if (getParam("contentItemNo") && filter.isBack) {
    connentBox.init();
  }
  if (this.pos == -1) {
...
filter.filterData = function (pindex, categoryId, state) {
  var url = 'p=yhScreenResultMulti&k=1&v=1&pindex=' + (pindex - 1) * 8 + '&psize=8&sizeFlag=1&catCode=' + filter.itemNo + '&categoryId=' + categoryId;
