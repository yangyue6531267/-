...
var getData = function (url) {
  ajax({
    ...
    success: function (response) {
      ...
      userBMS(uploadDom, contentID)//鉴权回调
...
//鉴权回调
function playAuthCallBack(res) {
  console.log('playAuthCallBack----' + JSON.stringify(res));
  uploadDom();
}
...
var playerBox = {
  ...
  onStop: function (res) {
    ...
    try {
      commonParams.asset_id = value.detailData.assetId;
      commonParams.item_id = value.detailData.itemList[indexSingle.itemNo].itemId.toString()
      commonParams.qb_datetime = Cookies.get('qb_datetime')
      commonParams.zb_datetime = (new Date()).valueOf().toString()
...
function registerLifecycleCallback(res) {
  ...
  try {
    if (res.status == 3 || res.status == 4) {
      ...
      his = JSON.parse(his);
      // player.stop();
      // player.play(his.playUrl, 0);

      playerBox.minPlay();
      player.initPlayer();
      player.toggleShow('showPlayer');
      player.setCallback(videoStateChange);
      player.play(his.playUrl);
    }
  } catch (err) {
...
//安徽局方收藏对接
function addFavorites(type) {
  var endTime = generateTimeReqestNumber(new Date())
  var contentId = getConetntId(value.detailData.assetId)
  var params = {
    accessToken: hex_md5(yh.accessKey + '_' + yh.userId),//AHYD_sjb_ott11
    cpName: yh.cpName,
    cmsId: yh.cspid,
    userId: yh.userId,
    userLicensor: yh.userLicensor,//用户当前牌照方：“BesTV”，“NewTV”
    apkUrl: yh.downloadAddress,//下载地址，如果apk不存在，触发下载安装。
    favorites: [{
      collectionFlag: type,
      contentType: '少儿',
      packageName: yh.apkName,
      className: yh.className,
      contentName: value.detailData.assetName,
      contentId: contentId,
      // extraContentId: 'YANHUA00000047PITEM0040007387059',
      imageUrl: value.detailData.assetImg,
      favoritesTime: endTime,
    }]
  }
  params = JSON.stringify(params);
  console.log('addFavorites----' + params);
  ajax({ // 入口数据
    type: "POST",
    url: ahHisUrl + '/ystAddFavorites',
    data: params,
    contentType: 'application/json',
    dataType: "json",
    success: function (data) {
      console.log('ahHisUrl 易视腾收藏发生完毕');
    },
    error: function (err) {
      console.log(err)
    }
  ...
  ajax({ // 入口数据
    type: "POST",
    url: ahHisUrl + '/bstAddFavorites',
    data: params,
    contentType: 'application/json',
    dataType: "json",
    success: function (data) {
      console.log('ahHisUrl百视通收藏发生完毕');
    },
    error: function (err) {
      console.log(err)
    }
...
function addBookmarks() {
  ...
  var params = {
    accessToken: hex_md5(yh.accessKey + '_' + yh.userId),//AHYD_sjb_ott11
    cpName: yh.cpName,//接口传过来
    cmsId: yh.cspid,
    userId: yh.userId,
    userLicensor: yh.userLicensor,//用户当前牌照方：“BesTV”，“NewTV”
    apkUrl: 'http://112.17.251.186:8080/apk/2019/12/13/159/159/20191213102227785whwnmmue.apk',//下载地址，如果apk不存在，触发下载安装。
    bookmarks: [{
      contentType: '少儿',
      packageName: yh.apkName,
      className: yh.className,
      contentName: value.detailData.assetName,//contentName
      contentId: contentId,//
      // extraContentId: 'YANHUA00000047PITEM0040007387059',
      // playType: '',//非必传
      playTime: parseInt(playerBox.detailData.curPlayTime / 1000),
      imageUrl: value.detailData.assetImg,//
      bookmarksTime: endTime,//
    }]
  }
  params = JSON.stringify(params);
  console.log('addBookmarks----' + params);
  ajax({ // 入口数据
    type: "POST",
    url: ahHisUrl + '/ystAddBookmarks',
    data: params,
    contentType: 'application/json',
    dataType: "json",
    success: function (data) {
      console.log('ahHisUrl易视腾历史记录发生完毕');
    },
    error: function (err) {
      console.log(err)
    }
  ...
  ajax({ // 入口数据
    type: "POST",
    url: ahHisUrl + '/bstAddBookmarks',
    data: params,
    contentType: 'application/json',
    dataType: "json",
    success: function (data) {
      console.log('ahHisUrl百视通历史记录发生完毕');
    },
    error: function (err) {
      console.log(err)
    }
